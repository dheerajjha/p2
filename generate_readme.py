...
def generate(db_path: str = "startups.db", out_path: str = "STARTUPS_ANALYSIS.md") -> None:
    ...
    a(f"| Median MRR | **{fmt_money(sorted(mrr_vals)[len(mrr_vals)//2] if mrr_vals else None)}** |")
    a(f"| Median asking price | **{fmt_money(sorted(price_vals)[len(price_vals)//2] if price_vals else None)}** |")
    a(f"| Median growth rate | **{fmt_pct(sorted(growth_vals)[len(growth_vals)//2] if growth_vals else None)}** |")
    a("")
    a("---")
    a("")
